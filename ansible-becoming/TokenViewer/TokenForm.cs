...
// Modified by @lowleveldesign: new logic for playing with DPAPI
    ...
    public partial class TokenForm : Form
    {
        ...
        private void btnEncrypt_Click(object sender, EventArgs e)
        {
            try {
                using (var token = _token.DuplicateToken(TokenType.Impersonation, SecurityImpersonationLevel.Impersonation, TokenAccessRights.MaximumAllowed)) {
                    using (var imp = token.Impersonate()) {
                        var plainText = Encoding.UTF8.GetBytes(txtDpapiContents.Text);
                        var cipherText = ProtectedData.Protect(plainText, null, DataProtectionScope.CurrentUser);
                        txtDpapiContents.Text = Convert.ToBase64String(cipherText);
                    }
                }
            } catch (Exception ex) {
                MessageBox.Show(this, ex.Message, $"Error: {ex.GetType().Name}", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnDecrypt_Click(object sender, EventArgs e)
        {
            try {
                using (var token = _token.DuplicateToken(TokenType.Impersonation, SecurityImpersonationLevel.Impersonation, TokenAccessRights.MaximumAllowed)) {
                    using (var imp = token.Impersonate()) {
                        var cipherText = Convert.FromBase64String(txtDpapiContents.Text);
                        var plainText = ProtectedData.Unprotect(cipherText, null, DataProtectionScope.CurrentUser);
                        txtDpapiContents.Text = Encoding.UTF8.GetString(plainText);
                    }
                }
            } catch (Exception ex) {
                MessageBox.Show(this, ex.Message, $"Error {ex.GetType().Name}", MessageBoxButtons.OK, MessageBoxIcon.Error);
